...
<script type="text/javascript">
	function deleteBook(id_book) {
		if (confirm("do you want to delete this book with id=" + id_book)) {
			window.location = "delete?id_book=" + id_book + "&&id_kh=1";
		}
	}
</script>
</head>
	...
	<div class="container-fluid pt-5">
		<div class="row px-xl-5">
			<c:if test="${account.role == 0 }">
				<a class="btn btn-primary col-lg-12 mb-2"
					href="bookdetail?id=${-1 }">Add Book</a>
			</c:if>
				...
				<table class="table table-bordered text-center mb-0">
					...
					<tbody class="align-middle">
						<c:forEach items="${list_books }" var="book">
							<tr>
								<td class="align-middle"><img src="${book.image }" alt=""
									style="width: 50px;"></td>
								<td class="align-middle">${book.title }</td>
								<td class="align-middle">${book.author }</td>
								<td class="align-middle">${book.category }</td>
								<td class="align-middle">${book.year }</td>
								<td class="align-middle">${book.sotrang }</td>
								<td class="align-middle">${book.ordered_amount }</td>
								<td class="align-middle"><c:if test="${account.role == 0 }">
										<div class="row">
											<div class="col">
												<a class="btn btn-primary btn-block mb-2"
													href="bookdetail?id=${book.id }">View</a>
											</div>
											<div class="col">
